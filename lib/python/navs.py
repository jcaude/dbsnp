...
TVcf = namedtuple(
    "TVcf", ["chrom", "pos", "id", "ref", "alt", "qual", "filter", "info"]
)
...
class Variation:
    ...
    def __init__(self, init_val=""):
        ...
        if rsid:
            ...
            m_other = re.fullmatch(
                "(?P<spdi>([^:]+:){3}[^:]+)?(?P<hgvs>[^:]+:[gcmnrp]\\.[^:]+)?(?P<vcf>([^\t]+\t){4}[^t].*)?",
                init_val,
            )
            if m_other:
    ...
    def __init_from_rsid(self, rsid):
        ...
        ptlp = self.__find_ptlp(
            self.rs["primary_snapshot_data"]["placements_with_allele"]
        )
        novar_spdi = dict()
        novar_hgvs = None
        novar_vcf_key = None
        ...
        for a in ptlp["alleles"]:
            ...
            if a["hgvs"].endswith("="):
                ...
                url = (
                    api_rootURL
                    + "spdi/"
                    + urllib.parse.quote(novar_spdi)
                    + "/vcf_fields"
                )
                n_vcf = json.loads(requests.get(url).text)[
                    "data"
                ]  # {chrom, pos, ref, alt}
                n_vcf["id"] = "rs" + str(rsid)  # {chrom, pos, id, ref, alt}
                novar_vcf_key = VcfT1(
                    **{f: str(n_vcf[f]) for f in VcfT1._fields}
                )  # (chrom, pos, id, ref)
            else:
                ...
                a_vcf = json.loads(requests.get(url).text)[
                    "data"
                ]  # {chrom, pos, ref, alt}
                a_vcf["id"] = "rs" + str(rsid)  # {chrom, pos, id, ref, alt}
                vcf_key = VcfT1(
                    **{f: str(a_vcf[f]) for f in VcfT1._fields}
                )  # (chrom, pos, id, ref)
        ...
        if not self.hgvs and novar_hgvs:
            ...
            vcf4rs[novar_vcf_key].append(
                "."
            )  # creates pair (chrom, pos, id, ref):['.']

        for vcf_key in vcf4rs:
            alt = ",".join(sorted(vcf4rs[vcf_key]))
            vcf_key2 = VcfT2(
                **{f: getattr(vcf_key, f) for f in ["chrom", "pos", "ref"]}, alt=alt
            )
    ...
    def __init_from_vcf(self, vcf):
        ...
        url = (
            api_rootURL
            + "vcf/"
            + "/".join(urllib.parse.quote(v) for v in list(vcf_key))
            + "/contextuals"
        )
